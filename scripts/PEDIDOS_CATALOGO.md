...
```
   ...
   └────────────┘                 └────────────┘
         │
         │ ① vence por fecha (rpc_expirar_pedidos_vencidos)
         │ ② campaña asociada cancelada (rpc_cancelar_campania)
         ▼
   ┌────────────┐
   │ EXPIRADO   │
   ...
   • Si EXPIRADO sin cancellation_reason  → POS muestra warning, deja cargar.
   • Si EXPIRADO con cancellation_reason  → POS BLOQUEA y muestra el motivo.
   • Si RECLAMADO                          → POS BLOQUEA con fecha de cobro.
```

### Campos relacionados (`product_kits`)
- `campania_ids uuid[]` — campañas que dieron descuento al pedido (puede ser
  vacío si todos los productos eran a precio normal).
- `cancellation_reason text` — motivo legible cuando una campaña asociada se
  canceló. Lo muestra el POS al cajero.
